...
import { Swapped, SubsidizedSwapped, SwappedTotal, User as UserEntity } from '../generated/schema'
...
const processSubsidizedEvent = (event: SwappedEvent): void => {
  if (event.params.settleAmount.gt(event.params.receivedAmount)) {
    ...
    const swappedTotalEntity = SwappedTotal.load('1')
...
const processSubsidizedTuppleEvent = (event: SwappedTuppleEvent): void => {
  ...
  if (txMetaData.settleAmount.gt(txMetaData.receivedAmount)) {
    ...
    entity.User_lastSeen = 0
    const userEntity = UserEntity.load(event.params.order.userAddr.toHex())
    if(userEntity !== null) {
      entity.User_lastSeen = userEntity.lastSeen
    }
