...
import { LimitOrderFilledByProtocol as LimitOrderFilledByProtocolEntity, LimitOrderFilledByTrader as LimitOrderFilledByTraderEntity, OrderCancelled as OrderCancelledEntity, Order as OrderEntity, LimitOrder as LimitOrderEntity, SwappedTotal as SwappedTotalEntity, User as UserEntity } from '../generated/schema'
...
export function handleLimitOrderFilledByProtocol(event: LimitOrderFilledByProtocolEvent): void {
  ...
  if (limitOrderEntity == null) {
    ...
    limitOrderEntity.SwappedTotal_total = BigInt.fromI32(0)
    const swappedTotalEntity = SwappedTotalEntity.load('1')
    if(swappedTotalEntity !== null) {
      limitOrderEntity.SwappedTotal_total = swappedTotalEntity.total
    }
    limitOrderEntity.User_lastSeen = 0
    const userEntity = UserEntity.load(event.params.maker.toHex())
    if(userEntity !== null) {
      limitOrderEntity.User_lastSeen = userEntity.lastSeen
    }
  }
